...
public class tp01p2q15{
	...
	public static void entrada()throws Exception{
		...
		presidentes.mostrar();
		Arq.openWrite("619148_bolha.txt");
	 ...
	 */
	 public void bolha() {
        for (int i = (n - 1); i > 0; i--) {
            for (int j = 0; j < i; j++) {
				comparacao++;
				int compara=array[j].getLocalNascimento().compareTo(array[j+1].getLocalNascimento());
                if (compara>0) {
					movimentacoes++;
					swap(j, j+1);
                }
				comparacao++;
				else if(compara==0&&array[j].getNome().compareTo(array[j+1].getNome())>0){
						movimentacoes++;
						swap(j,j+1);
				}
            }
