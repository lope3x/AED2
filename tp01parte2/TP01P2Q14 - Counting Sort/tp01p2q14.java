...
class Lista{
	...
	*/
	...
	/**
	 * O metodo realiza a ordenacao do array utilizando o algoritmo do CoutingSort
	 */
	 public void countingSort(){
		 int[] counting=new int[maiorIdade()+1];
		 Presidente[] output=new Presidente[n+1];
		 int iguais=0;
		 //preenche o couting com 0
		 for(int i=0;i<counting.length;i++)counting[i]=0;
		 //contabiliza quantas vez cada valor repete no array
		 for(int i=0;i<n;i++){
			 counting[array[i].getIdade()]++;
		 }
		 //atualiza counting
		 ...
		 //ordena
		 for(int i=0;i<n;i++){
			 int posout=counting[array[i].getIdade()];
			 output[posout]=array[i];
			 counting[array[i].getIdade()]--;
			 movimentacoes++;
		 }
		 ...
		 for(int i=0;i<n-1;i++){
			 ...
			 //151
			 //178
			 int idade = array[i].getIdade();
			 switch(idade){
				 case 89:
					comparacao++;
					if(array[i].getNome().compareTo(array[i+1].getNome())>0)swap(i,i+1);
					i++;
					break;
				 case 120:
					comparacao++;
