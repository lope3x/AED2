	...
	*/
	...
	public void ordenacaoInsercao(){
		for(int i=1;i<n;i++){
			...
			int j=i-1;//j ordenada i desordenada
	...
	/**
	 * O metodo ordena o array parcialmente utilizando o algoritmo de insercao com k igual a 10
	 */
	public void ordenacaoInsercaoParcial(){
		int inicio;
		for(int i=1;i<n;i++){
			Presidente tmp=array[i];
			if(i>(10-1)){
			...
			int j=inicio;//j ordenada i desordenada
			while((j>=0)&&(array[j].getPaginaTam()>tmp.getPaginaTam())){
				comparacao++;
				movimentacoes++;
				array[j+1] = array[j];
