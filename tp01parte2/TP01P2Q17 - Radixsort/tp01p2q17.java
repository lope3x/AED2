...
class Lista{
	...
	*/
	...
	/**
	 * O metodo realiza a ordenacao do array por meio do radixSort
	 */
	public void radixSort()throws Exception{
		int maior= procuraMaiorId();
		int digitos=qtosDig(maior);
		int dig;
		Presidente[][] buckets = new Presidente[10][maior];
		//repete digitos vezes
		for(int i=0;i<digitos;i++){
			//percorre a lista 
			for(int y=0;y<n;y++){
				dig=pegaDig(i+1,array[y].getId());
				buckets[dig][y]=array[y];
				//MyIO.println("AQUII");
			}
			//x faz a funcao de i
			int pos=0;
	...
	/**
	 * O metodo procura o maior id de Presidente no vetor
	 * @return int maior;
	 */
	public int procuraMaiorId(){
	...
	/**
	 * O metodo verifica quantos digitos o numero recebido tem
	 * @param int num numero para a verificacao
	 * @return int cont numero de digitos 
	 */
	public int qtosDig(int num){
		int cont=0;
		while(num !=0) {
			num = num/10;
			cont++;
	...
	/**
	 * O metodo recebe um numero inteiro n e um outro inteiro dig com indicando qual digito a ser verificado e retorna o valor desse digito
	 * @param int dig posicao a ser verificada
	 * @param int n
	 * @return int resp o numero que se encontra no digito verificado
	 */
	public int pegaDig(int dig,int n){
		int divisor=1;
		int resp;
		for(int i=1;i<dig;divisor*=10,i++);
		n=n/divisor;
		resp=n%10;
		return resp;
