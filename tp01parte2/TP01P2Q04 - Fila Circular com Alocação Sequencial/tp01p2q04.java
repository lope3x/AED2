   ...
   public static void entrada()throws Exception{
	   String input=MyIO.readLine();
		...
		do{
			...
			presidentes.inserirFila(teste.getClone());
			MyIO.println(presidentes.getMediaIdade());
			input=MyIO.readLine();
		...
		for(int i=0;i<n;i++){
				...
				if(input.equals("I")){
					...
					tmp.setPagina(input);
					presidentes.inserirFila(tmp.getClone());
					MyIO.println(presidentes.getMediaIdade());
					tmp=null;
				}
				if(input.equals("R")){
					removido=presidentes.removerFila();
...
class Fila{
    ...
    */
   public void inserirFila(Presidente x)throws Exception{
   
      //caso esteja cheia remove um elemento e prossegue com a insercao
      if (isFull()) {
        this.removerFila();
      }
    ...
    */
   public Presidente removerFila() throws Exception {
   ...
   public void mostrar ()throws Exception{
		if(isVazia()){
			throw new Exception("Erro ao mostrar Array esta Vazio");
		}
	...
	* @return boolean indicando se afila esta cheia 
   */
   public boolean isFull(){
		return (((ultimo+1)%array.length)==primeiro);
   }
   /**
	* Retorna um inteiro com a media das idades dos presidentes ja inseridos na fila
	* @return int com o valor da media das idades
   */
	public int getMediaIdade(){
		...
		int resp;
		for(int i=primeiro;i!=ultimo;i=((i+1)%array.length)){
			idades+=array[i].getIdade();
			c++;
		}
		resp=(int)Math.round(idades/c);
		return resp;
	}
}
