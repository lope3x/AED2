...
public class tp01p2q07{
	...
	public static void entrada()throws Exception{
		...
		do{
			if(presidentes.pesquisaBinaria(input))MyIO.println("SIM");
			else MyIO.println("N�O");
		...
		Arq.openWrite("619148_binaria.txt");
		long tempo=(fim-comeco);
		Arq.print("619148\t"+tempo+"\t"+comparacao);
		Arq.close();	
	}
	/*	
		@author Gabriel Lopes
	...
	*/
	...
	public boolean pesquisaBinaria(String nome){
		...
		while(esq<=dir){
			meio=(esq+dir)/2;
			comparacao++;
			int compara=nome.compareTo(array[meio].getNome());
			if(compara==0){
				resp=true;
				esq=n;
			}	
