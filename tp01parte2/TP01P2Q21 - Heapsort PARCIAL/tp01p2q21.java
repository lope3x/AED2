...
class Lista{
	...
	*/
	...
	/**
	 * Mostra o K primeiros elementos de um vetor
	 * @param int k referente aos k primeiros elementos
	 */
	...
	public boolean pesquisaBinaria(String nome){
		...
		while(esq<=dir){
			...
			if(nome.equals(array[meio].getNome())){
				...
				comparacao++;
				if(nome.compareTo(array[meio].getNome())>0){
					esq=meio+1;
				}
				else{
					dir=meio-1;
	...
	/**
	 * O metodo realiza o loop interno do algoritmo de selecao recursiva
	 */
	...
	/**
	 * O metodo ordena o array utilizando o algoritmo de insercao
	 */
	...
	/**
	 * O metodo ordena o array utilizando o algoritmo shellsort
	 */
	...
	/**
	 *Metodo que efetua a insercao dos pseudo-arrays do shellsort
	 * @param int cor cor do pseudo array
	 * @param int h passo do shellsort
	 */
	 //nome.compareTo(array[meio].getNome())>0//proximo > z  ** < a
	public void insercaoCor(int cor,int h){
	...
	/**
	 * O metodo realiza a ordenacao do array utilizando o algoritmo do QuickSort
	 */
	...
	public void quickSort(int esq,int dir){
		int i=esq,j=dir;
		LocalDate pivo=array[(esq+dir)/2].getInicioMandato();
		while(i<=j){
			while(array[i].getInicioMandato().isBefore(pivo)){
				i++;
			}
			while(array[j].getInicioMandato().isAfter(pivo)){
				j--;
			}
			comparacao+=2;
			if(i<=j){
	...
	/**
	 * @author Gabriel Lopes
	 * @param int tamheap o tamanho do heap a ser construido
	 */
	public void buildHeap(int tamheap){
		...
		for(int k=0;k<tamheap;k++){
			for(int i=k+1;i>=-0;i=(i-1)/2){
				comparacao++;
				if(array[i].getDataNascimento().isAfter(array[(i-1)/2].getDataNascimento())){
					swap(i,(i-1)/2);
				}
				else{
					i=-1;
	...
	/**
	 * @author Gabriel Lopes
	 * @param int tamheap o tamanho do heap a ser reconstruido
	 */
	public void reBuildHeap(int tamheap){
		//comparar irmãos
		//k eh o pai
		int filho;//sempre sera o maior filho
		for(int k = 0;k<=(tamheap-2)/2;){
			comparacao++;
			if(array[(2*k)+1].getDataNascimento().isAfter(array[(2*k)+2].getDataNascimento())||(2*k)+1==tamheap){
				filho=(2*k)+1;
			}	
			else{ 
				filho=(2*k)+2;
			...
			comparacao++;
			if(array[k].getDataNascimento().isBefore(array[filho].getDataNascimento())){
				swap(k,filho);
				k=filho;
			}
			else{
	...
	/**
	 * O metodo ordena o array utlizando o algoritmo heapsort
	 */
	...
	/**
	 * O metodo reconstroi o heap 
	 * @param int tamheap tamanho do heap a ser reconstruido
	 */
	public void reBuildHeapParcial(int tamheap){
		//comparar irmãos
		//k eh o pai
		int filho;//sempre sera o maior filho
		int pai=0;
		for(int k = 0;k<=(tamheap-2)/2;){
			comparacao++;
			if(array[(2*k)+1].getDataNascimento().isBefore(array[(2*k)+2].getDataNascimento())||(2*k)+1==tamheap){
				filho=(2*k)+1;
			}	
			else{ 
				filho=(2*k)+2;
			...
			comparacao++;
			if(array[k].getDataNascimento().isAfter(array[filho].getDataNascimento())){
				swap(k,filho);
				k=filho;
			}
			else{
	...
	/**
	 * O metodo constroi um heap
	 * @param int tamheap tamanho do heap a ser construido
	 */
	public void buildHeapParcial(int tamheap){
		for(int k=0;k<tamheap;k++){
			for(int i=k+1;i>=-0;i=(i-1)/2){
				comparacao++;
				if(array[i].getDataNascimento().isBefore(array[(i-1)/2].getDataNascimento())){
					swap(i,(i-1)/2);
				}
				else{
					i=-1;
	...
	/**
	 * O metodo ordena o array ate k elementos com o algoritmo do heapsort
	 */
	public void heapSortParcial(int k){
		int ultimo=n;
	...
	/**
	 * Metodo auxiliar do algoritmo heapsort , o metodo inverte o array
	 */
