...
int mov=0;
int cmp=0;
//verifica se a primeira data é maior q a segunda
...
int swap(int i,int j){
	struct Presidente tmp =presidentes.array[i];
	presidentes.array[i]=presidentes.array[j];
	presidentes.array[j]=tmp;
	mov+=3;
	return 0;
}
...
void quicksortRec (int esq, int dir) {
   int i = esq, j = dir;
   struct data pivo = presidentes.array[(dir+esq)/2].inicioMandato;
   while (i <= j) {
      while (comparaDatas(pivo,presidentes.array[i].inicioMandato)) {
		  cmp++;
		  i++;
	  }
      while (comparaDatas(presidentes.array[j].inicioMandato,pivo)){
		cmp++;	
		 j--;
	  }
      if (i <= j) {
...
void quicksort() {
   quicksortRec(0, presidentes.n-1);
}
...
int main(){
	...
	int repete;
	int i=0;
	while(!equals(arquivo,fim)){
		scanf("%s",arquivo);
		if(!equals(arquivo,fim)){
			presidentes.array[i] = ler(arquivo);
			presidentes.n++;
			i++;
		}
	}
	FILE *p =fopen("619148_quicksort.txt","w");
