...
class ListaDuplaFlexivel{
	...
	public void quicksort(CelulaDupla esq,CelulaDupla dir,int posesq,int posdir){
		...
		LocalDate pivo=dir.elemento.getInicioMandato();//seta pivo como ultimo elemento
		while(posi<=posj){
			while(i.elemento.getInicioMandato().isBefore(pivo)){
				i=i.prox;
				posi++;
			}
			while(j.elemento.getInicioMandato().isAfter(pivo)){
				j=j.ant;
				posj--;
			}
			if(posi<=posj){
