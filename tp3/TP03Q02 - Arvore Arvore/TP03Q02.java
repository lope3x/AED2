...
public class TP03Q02{
	...
	public static void entrada()throws Exception{
		...
		Arq.openWrite("619148_arvoreArvore.txt");
...
class ArvoreArvore{
	...
	private boolean pesquisar(String nome,No i){
		boolean resp =false;
		if(i!=null){
			resp = pesquisarInterna(nome,i.raiz);
			if(!resp){
	...
	private boolean pesquisarInterna(String nome,No2 i){
		//alterar isso para organizar pela idade
		boolean resp = false;
		if(i!=null){
			cmp++;
			...
			if(!resp){
				System.out.print("ESQ ");
				resp = pesquisarInterna(nome,i.esq);
				if(!resp){
					System.out.print("DIR ");
					resp = pesquisarInterna(nome,i.dir);
				}
			}
		}
		return resp;
