...
public class TP03Q08{
	...
	public static void entrada()throws Exception{
		...
		do{
			...
			tmp.lePresidente(input);
			tmp.setPagina(input);
			presidentes2.inserir(tmp.getNome());
			input=MyIO.readLine();
			if(!input.contains("FIM"))input=UTF8toISO(input);
		}while(!input.contains("FIM"));
		trie presidentes3 = trie.merge(presidentes1,presidentes2);
		input=MyIO.readLine();
		input=UTF8toISO(input);
		//presidentes3.mostrar();
		long comeco=System.currentTimeMillis();
		do{
			//(presidentes.pesquisar(input)) ? MyIO.println("SIM") : MyIO.println("NÃO"); 
			//System.out.print(input);
			MyIO.println((presidentes3.pesquisar(input)) ? "SIM" : "N�O");
			input=MyIO.readLine();
			if(!input.contains("FIM"))input=UTF8toISO(input);
		}while(!input.contains("FIM"));
		...
		Arq.print("619148\t"+tempo+"\t"+presidentes3.cmp);
		Arq.close();
	}
	/*	
		@author Gabriel Lopes
